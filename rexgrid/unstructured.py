...
import numpy as np
from xugrid import Ugrid2d, voronoi
...
class UnstructuredGrid2d:
    ...
    def barycentric(self, other):
        ...
        # Triangulate the grid if above lines are uncommented. This shouldn't
        # be necessary: Wachspress barycentric weights give nicer results for
        # convex polygons.
        # However, this is a workaround for:
        # https://github.com/Deltares/numba_celltree/issues/2
        # It's not a guaranteed workaround, however, as I do not quite
        # understand the bug yet...

        voronoi_triangulated_grid = Ugrid2d(
            vertices[:, 0],
            vertices[:, 1],
            -1,
            faces,
        ...
        face_index, weights = voronoi_triangulated_grid.compute_barycentric_weights(
            points
        )
        n_points, n_max_node = weights.shape
        keep = weights.ravel() > 0
        source_index = node_to_face_index[
            voronoi_triangulated_grid.face_node_connectivity[face_index]
        ].ravel()[keep]
