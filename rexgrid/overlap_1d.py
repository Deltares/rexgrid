...
def _find_indices(side):
    ...
    def find_indices(
        ...
        indices = np.full((source_index.size, m), -1, dtype=np.intp)
        sorter = np.empty(n, np.intp)
...
def overlap_1d_nd(
    ...
    target_index_d = np.repeat(
        np.broadcast_to(
            np.arange(target_d_size, dtype=np.intp), target_bounds.shape[:2]
        ).ravel(),
        n_overlap.ravel(),
    )

    # Create source index
    increment = alt_cumsum(np.ones(n_total, dtype=np.intp)) - np.repeat(
